...
* **RAG (検索拡張生成) 機能**: Qiita記事の埋め込みベクトルをSupabaseに保存し、Slack Botへの質問応答に活用。
...
* **Slack Bot**: Slack Bolt Framework (`slack_sdk`)
* **Core APIs & Libraries**:
...
│   ├── rag_indexer.py        # 記事のベクトル化とSupabaseへの保存処理
...
| `SUPABASE_URL` | Supabase プロジェクト URL | ✅ 必須 (RAG用) |
| `SUPABASE_KEY` | Supabase API Key (service_role ではなく anon/public 推奨) | ✅ 必須 (RAG用) |
